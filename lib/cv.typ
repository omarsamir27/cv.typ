...
#let contacttext(info, uservars) = block(width: 100%)[
  ...
  #if info.personal.at("profiles", default: none) != none and info.personal.profiles.len() > 0 {
...
#let cvobjective(info, title: "Objective") = {
  let objective = info.at("objective", default: none)
  if objective != none {
    block()[
      == #title
      #objective
          ...
          block(width: 100%, breakable: isbreakable, above: 0.6em)[
            ...
            // Highlights or Description
              ...
              for hi in p.highlights [
                #if hi.at("url", default: none) != none {
                  list(link(hi.at("url"))[#eval(hi.text, mode: "markup")])
                } else {
                  list(eval(hi.text, mode: "markup"))
                }
    ...
    block[
      ...
      #for edu in info.education {
        ...
        block(width: 100%, breakable: isbreakable)[
          ...
          #text(style: "italic")[#edu.studyType in #edu.area #edu.at("score",default: none)] #h(1fr)
          #utils.daterange(start, end) \
  ...
  if info.projects != none {
    block[
      ...
      #for project in info.projects {
        // Parse ISO date strings into datetime objects
        let start = utils.strpdate(project.at("startDate", default: none))
        let end = utils.strpdate(project.at("endDate", default: none))
        ...
        let name = if is.has("url", project) {
          [‣ *#link(project.url)[#project.name]*]
        } else {
          [‣ *#project.name*]
        }
        let affiliation = if is.has("affiliation", project) {
          [#text(style: "italic")[#project.affiliation]]
        }

        let project_disp = utils.array2grid(
          2,
          (
            name,
            [],
  ...
  if info.certificates != none {
    block[
      ...
      #for cert in info.certificates {
        // Parse ISO date strings into datetime objects
        let date = if is.has("date", cert) {
          utils.strpdate(cert.date)
        } else {
          []
        }

        let name = if is.has("url", cert) {
          [‣ #link(cert.url)[#eval(cert.name, mode: "markup")]]
        } else {
          [‣ #eval(cert.name, mode: "markup") ]
        }

        let id = if is.has("id", cert) {
          [ID: #raw(cert.id)]
        } else {
          []
        }

        let issuer = if is.has("issuer", cert) {
